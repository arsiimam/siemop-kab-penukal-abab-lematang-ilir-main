...
class ReportController extends Controller
{
    ...
    public function looping($val)
    {
        ...
        array_push($this->list, [
            ...
            // $val->non_fisik != null ? number_format($val->non_fisik) : '',
            $realization,
    ...
    public function generate_pdf(Request $request)
    {
        try {
            ...
            if ($request->dinas_id != '') {
                ...
                $institute = Institute::find($dinas_id);
                ...
                if (Auth::user()->can('Manage Any Report')) {
                    ...
                    // signature default from sistem
                    $sign_institute = settingByUnique('company_name');
                    if (file_exists(settingByUnique('setting_signature_file'))) {
                        $sign_image = settingByUnique('setting_signature_file');
                    } else {
                        $sign_image = '';
                    }
                    $sign_name = settingByUnique('setting_name_signature');
                    $sign_position = settingByUnique('setting_signature_position');
                    $sign_nip = settingByUnique('setting_signature_nip');
                } else {
                    $dinas_id = Auth::user()->institute_id;
            ...
            $pdf = Pdf::loadView('backend.report.pdf_realisasi_fisik', compact('data', 'tahun', 'month', 'signature'))->setPaper('A4', 'landscape');
