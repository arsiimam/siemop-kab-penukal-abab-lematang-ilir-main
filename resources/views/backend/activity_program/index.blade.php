...
@section('header')
    <h1>{{ __('Program Kerja') }}</h1>
@endsection

@section('action-button')
    <div class="top-right-button-container">
        <div class="btn-group" role="group">
            @can('Create Activity Program')
                <button type="button" class="btn btn-primary top-right-button mb-2 mr-1 modal-btn" style="width: 80px;"
                    data-title="{{ __('Tambah Data Baru') }}" data-url="{{ route('activity-program.create') }}" data-toggle="modal"
                    data-backdrop="static">{{ __('Tambah') }}</button>
            @endcan

            @can('Delete Activity Program')
...
@push('scripts')
    ...
    <script>
        const table = $('#activity-program-table')
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            url: "{{ url('api/activity-program/bulk-delete') }}",
                            type: "post",
                            data: {
                                id: deleteids_arr
                            },
